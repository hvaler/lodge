 ...
 */
export function capabilitiesFor(config: StandardsConfig): Capability[] {
  const capabilities: Capability[] = [];

  if (config.inventory) {
    capabilities.push('rooms');
    // Directions are derived from the same table: a room knows its building and floor.
    capabilities.push('wayfinding');
  }
  if (config.calendars?.deadlines) capabilities.push('deadlines');
  if (config.calendars?.timetable && config.directory) capabilities.push('timetable');

  return capabilities;
}

export function descriptorFor(config: StandardsConfig): ProviderDescriptor {
  return {
    id: 'standards',
...
export function assertConfigUsable(config: StandardsConfig): void {
  ...
  if (!config.inventory) missing.push('an `inventory` CSV would enable room search and directions');
