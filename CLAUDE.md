...
>   `.claude/CLAUDE_BASE.md`. La regla `estandares-base` lo recuerda al tocar código de `src/`.
...
> Precedencia de configuración y claves: `.claude/CONFIGURACION.md`.

> Claude consulta **Hilo** para el estado actual del proyecto y **Patrón** para documentacion de referencia.
...
>
> Lo que sí se publica y sí es de Lodge: [`_hilo/DECISIONES.md`](_hilo/DECISIONES.md), las veinte
...
> `.claude/CLAUDE_BASE.md` recoge el estándar de la organización, que es **.NET/SQL Server**: aplica
...
6. **JSON del Hilo**: un script Node que escriba `_hilo/*.json` usa `writeHiloJson` de
   `.claude/hooks/lib/helpers.js`; en PowerShell, `[System.IO.File]::WriteAllText` con UTF-8 sin BOM
